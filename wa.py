...
START_VAX_PROJECTIONS = np.argwhere(dates==np.datetime64('2022-01-22'))[0][0]
all_dates = dates
all_new = new
...
START_PLOT = np.datetime64('2021-12-20')
...
padding_model = lambda x, A, k: exponential_with_infection_immunity(
    ...
    tau=tau,
    effective_population=TEST_DETECTION_RATE * POP_OF_WA,
)
...
if VAX:
    title_lines = [
        f"SIR model of Western Australia as of {latest_update_day}",
        f"Starting from currently estimated {R_eff_string}",
    ]
else:
    region = "Western Australia"
    title_lines = [
        f"$R_\\mathrm{{eff}}$ in {region} as of {latest_update_day}, with Perth restriction level and daily cases",
...
text = fig1.text(
    0.99,
    0.02,
    "@chrisbilbo | chrisbillington.net/COVID_WA",
    size=8,
    alpha=0.5,
    color=(0, 0, 0.25),
...
if OLD:
    fig1.savefig(f'wa_animated/{OLD_END_IX:04d}.png', dpi=133)
else:
    fig1.savefig(f'COVID_WA{suffix}.svg')
    fig1.savefig(f'COVID_WA{suffix}.png', dpi=133)
if True: # Just to keep the diff with nsw.py sensible here
    ...
    if OLD:
        fig1.savefig(f'wa_animated_linear/{OLD_END_IX:04d}.png', dpi=133)
    else:
        fig1.savefig(f'COVID_WA{suffix}_linear.svg')
        fig1.savefig(f'COVID_WA{suffix}_linear.png', dpi=133)

# Save some deets to a file for the auto reddit posting to use:
try:
    # Add to existing file if already present
    stats = json.loads(Path("latest_wa_stats.json").read_text())
...
if not OLD:
    ...
    Path("latest_wa_stats.json").write_text(json.dumps(stats, indent=4))

    # Update the date in the HTML
    html_file = 'COVID_WA.html'
    html_lines = Path(html_file).read_text().splitlines()
    now = datetime.now(timezone('Australia/Perth')).strftime('%Y-%m-%d %H:%M')
    for i, line in enumerate(html_lines):
        if 'Last updated' in line:
            html_lines[i] = f'    Last updated: {now} Perth time'
