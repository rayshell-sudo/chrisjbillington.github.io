...
dates, new = covidlive_case_data('TAS', start_date=np.datetime64('2021-11-20'))
...
if dates[-1] >= np.datetime64('2022-01-04'):
    padding_model = lambda x, A, k: exponential_with_infection_immunity(
        ...
        tau=tau,
        effective_population=TEST_DETECTION_RATE * POP_OF_TAS,
    )
else:
...
if OLD:
    fig1.savefig(f'tas_animated/{OLD_END_IX:04d}.png', dpi=133)
else:
    fig1.savefig(f'COVID_TAS{suffix}.svg')
    fig1.savefig(f'COVID_TAS{suffix}.png', dpi=133)
if True:  # Just to keep the diff with nsw.py sensible here
    ...
    if OLD:
        fig1.savefig(f'tas_animated_linear/{OLD_END_IX:04d}.png', dpi=133)
    else:
        fig1.savefig(f'COVID_TAS{suffix}_linear.svg')
        fig1.savefig(f'COVID_TAS{suffix}_linear.png', dpi=133)

# Save some deets to a file for the auto reddit posting to use:
try:
    # Add to existing file if already present
    stats = json.loads(Path("latest_tas_stats.json").read_text())
...
if not OLD:
    ...
    Path("latest_tas_stats.json").write_text(json.dumps(stats, indent=4))

    # Update the date in the HTML
    html_file = 'COVID_TAS.html'
    html_lines = Path(html_file).read_text().splitlines()
    now = datetime.now(timezone('Australia/Hobart')).strftime('%Y-%m-%d %H:%M')
    for i, line in enumerate(html_lines):
        if 'Last updated' in line:
            html_lines[i] = f'    Last updated: {now} Hobart time'
