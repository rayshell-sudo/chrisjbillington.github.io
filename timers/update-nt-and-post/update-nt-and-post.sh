...
# Wait for NT data to become available:
if python wait-for-nt-update.py | grep "ready!"; then
    ./nt.sh
fi

# Post to reddit:
python post-nt-to-reddit.py \
  "${REDDIT_CLIENT_ID}" \
  "${REDDIT_CLIENT_SECRET}" \
  "${REDDIT_PASSWORD}"

# Post to twitter:
python post-nt-to-twitter.py \
  "${TWITTER_API_KEY}" \
...
nice python animate-nt.py

git commit --all -m "NT animation"
