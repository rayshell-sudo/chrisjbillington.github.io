...
# Wait for SA data to become available:
if python wait-for-sa-update.py | grep "ready!"; then
    ./sa.sh
fi

# Post to reddit:
python post-sa-to-reddit.py \
  "${REDDIT_CLIENT_ID}" \
  "${REDDIT_CLIENT_SECRET}" \
  "${REDDIT_PASSWORD}"

# Post to twitter:
python post-sa-to-twitter.py \
  "${TWITTER_API_KEY}" \
