...
if dates[-1] >= np.datetime64('2022-01-04'):
    padding_model = lambda x, A, k: exponential_with_infection_immunity(
        ...
        tau=tau,
        effective_population=TEST_DETECTION_RATE * POP_OF_ACT,
    )
...
if not OLD:
    ...
    now = datetime.now(timezone('Australia/Canberra')).strftime('%Y-%m-%d %H:%M')
    for i, line in enumerate(html_lines):
        if 'Last updated' in line:
            html_lines[i] = f'    Last updated: {now} Canberra time'
