...
dates, new = covidlive_new_cases('QLD', start_date=np.datetime64('2021-11-20'))
...
if dates[-1] >= np.datetime64('2022-01-04'):
    padding_model = lambda x, A, k: exponential_with_infection_immunity(
        ...
        tau=tau,
        effective_population=TEST_DETECTION_RATE * POP_OF_QLD,
    )
else:
...
if VAX:
    title_lines = [
        f"SIR model of Queensland as of {latest_update_day}",
...
text = fig1.text(
    0.99,
    0.02,
    "@chrisbilbo | chrisbillington.net/COVID_QLD",
    size=8,
    alpha=0.5,
    color=(0, 0, 0.25),
...
if OLD:
    fig1.savefig(f'qld_animated/{OLD_END_IX:04d}.png', dpi=133)
else:
    fig1.savefig(f'COVID_QLD{suffix}.svg')
    fig1.savefig(f'COVID_QLD{suffix}.png', dpi=133)
if True: # Just to keep the diff with nsw.py sensible here
    ...
    if OLD:
        fig1.savefig(f'qld_animated_linear/{OLD_END_IX:04d}.png', dpi=133)
    else:
        fig1.savefig(f'COVID_QLD{suffix}_linear.svg')
        fig1.savefig(f'COVID_QLD{suffix}_linear.png', dpi=133)
...
if not OLD:
    ...
    Path("latest_qld_stats.json").write_text(json.dumps(stats, indent=4))
